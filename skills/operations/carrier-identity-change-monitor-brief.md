...
- Reference `knowledge-base/best-practices/broker-carrier-vetting-liability.md` for the post-Montgomery v. Caribe Transport II negligent-hiring overlay — a change-of-ownership event that coincides with a safety-rating downgrade or a BASIC percentile crossing the broker's intervention threshold is the worst-case shape: tendering through it carries both the fraud-loss exposure this brief is designed around and the negligent-hiring exposure the overlay is designed around. Pin the safety-rating-and-BASIC snapshot in the disposition rationale when an ownership or behavior change is observed
